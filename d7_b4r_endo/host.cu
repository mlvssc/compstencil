   ...
   while(nn>0)
   {
        ...
        for (blk_i=0;blk_i<n_div;blk_i++)
	{
		...
		cudaMemcpyAsync(buf_A[si]+d_start, A1+h_start, trans_len*sizeof(SB_TYPE), cudaMemcpyHostToDevice, pipes[si]);
	     ...
	     {
        	#define __c0 c0
        	AN5D_TYPE __c1Len = buf_zlen - 4 - 4;// = (dimsize - 4 - 4);
        	AN5D_TYPE __c1Pad = (4);

        	#define __c1 c1
                AN5D_TYPE __c2Len = (dimsize - 4 - 4);
        	AN5D_TYPE __c2Pad = (4);
        	#define __c2 c2
        	AN5D_TYPE __c3Len = (dimsize - 4 - 4);
        	AN5D_TYPE __c3Pad = (4);
        	#define __c3 c3
        	AN5D_TYPE __halo1 = 4;
        	AN5D_TYPE __halo2 = 4;
        	...
        	AN5D_TYPE __side1Len = 256;
        	AN5D_TYPE __side2Len = 8;
        	AN5D_TYPE __side3Len = 56;
		...
		for (c0 = 0; c0 < tb_tps; c0 += 1)
			...
			//unsigned int lbound = 2*BENCH_RAD+l_blk-13;
			//if(l_blk!=0&&lbound<17&&n_zblk!=1)	n_zblk--;								
			//printf("c1len=%d, nb=%d -->",__c1Len,n_zblk);
			...
			dim3 k0_dimGrid(1 * n_zblk * ((__c2Len + __side2Len - 1) / __side2Len) * ((__c3Len + __side3Len - 1) / __side3Len), 1, 1);
          		...
          		//kernel0_1<<<k0_dimGrid, k0_dimBlock, 0, pipes[si]>>> (buf_A[si], dimsize, timestep, c0);
