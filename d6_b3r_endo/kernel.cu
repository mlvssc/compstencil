...
__global__ void kernel0_1_pipe(double *A, int dimsize, int zlen,int timestep, int c0, int blktype)
{
    ...
    #endif
    #define __c0 c0
    AN5D_TYPE comp_z = zlen - (1+timestep)*3;
    AN5D_TYPE __c1Len = comp_z;
    AN5D_TYPE __c1Pad = (timestep-__c0)*3;

    if(blktype==0) //first blk
    {
	__c1Len = (timestep-1-__c0)*3 + comp_z;
	__c1Pad = 3;
     }	    
    else if(blktype==1) // last blk
    {
	__c1Len = (1-timestep+__c0)*3+comp_z;
     }
    ...
    AN5D_TYPE __side1Num = (__c1Len + __side1Len - 1) / __side1Len;
    AN5D_TYPE l_blk = __c1Len%__side1Len;
    if(l_blk!=0&&__side1Num!=1) __side1Num--;
    //printf("%d %d %d--",__c1Pad,__c1Len,__side1Num);
