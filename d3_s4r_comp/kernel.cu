#include <stdio.h>
#include "kernel.hu"
__device__ SB_TYPE __sbref_wrap(SB_TYPE *sb, size_t index) { return sb[index]; }

__global__ void kernel0_1_pipe(SB_TYPE *A, int dimsize, int zlen,int timestep, int c0, int blktype)
{
#ifndef AN5D_TYPE
#define AN5D_TYPE unsigned
#endif
    #define __c0 c0
    AN5D_TYPE comp_z = zlen - (1+timestep)*4;
    AN5D_TYPE __c1Len = comp_z;
    AN5D_TYPE __c1Pad = (timestep-__c0)*4;

    if(blktype==0) //first blk
    {
	    __c1Len = (timestep-1-__c0)*4 + comp_z;
	    __c1Pad = 4;
    }	    
    else if(blktype==1) // last blk
    {
	    __c1Len = (1-timestep+__c0)*4+comp_z;
    }
    //if(threadIdx.x==0)
    //	printf("type=%d pad=%d len=%d comp_z=%d\n",blktype,__c1Pad, __c1Len,comp_z);
    //return;
    #define __c1 c1
    ...
    const AN5D_TYPE __side1Len = 64;//128;
    ...
    SB_TYPE __reg_0_7;
    SB_TYPE __reg_0_8;
    __shared__ SB_TYPE __e_sb_double[__blockSize * 2];
    SB_TYPE *__e_sb = __e_sb_double;
    ...
    //#define __CHECKADDR() do { if((((((c0 + 1) % 2) * zlen + c1) * dimsize + c2) * dimsize + c3)<0||(((((c0 + 1) % 2) * zlen + c1) * dimsize + c2) * dimsize + c3)>=2*zlen*dimsize*dimsize) {printf("c1=%d c2=%d c3=%d\n", c1, c2, c3);} return; } while (0) 
    if (__c1Id == 0)
    {
      __LOAD(__reg_0_0, 0);
      __LOAD(__reg_0_1, 1);
